...
def verify_license(source: dict[str, Any]) -> dict[str, Any]:
    ...
    commercial_ok = source.get("commercial_use_status") == "allowed" and license_id in COMMERCIAL_LICENSES
    allowed_for_sample = classification in APPROVED_STATUSES and explicit and source.get("commercial_use_status") in {"allowed", "allowed_with_attribution"}
    return {
...
def estimate_source_download(source: dict[str, Any], *, sample_only: bool = True) -> dict[str, Any]:
    size = source.get("sample_size_bytes") if sample_only else source.get("dataset_size_bytes")
    return {
        "source_id": source["source_id"],
...
def assert_source_download_allowed(source: dict[str, Any], *, full_dataset: bool = False, max_bytes: int) -> None:
    ...
    if not license_result["sample_download_allowed"]:
        raise PermissionError(f"Sample download blocked by license classification: {source.get('classification')}")
    if source.get("requires_authentication") or source.get("requires_form") or source.get("requires_account"):
        raise PermissionError("This source requires authentication, account creation, or form/terms acceptance.")
